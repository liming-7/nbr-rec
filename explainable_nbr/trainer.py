...
class Trainer(object):
    ...
    def __init__(self, config, model):
        ...
        self.meta_loss = config['meta_loss']
    ...
    def _train_epoch(self, train_data, epoch_idx, show_progress=False):
        ...
        for batch_id in range(total_batch_cnt):
            candidates = dict()
            data_train, tgt_train, repeat_data, explore_data, item_data, user_data, pop_data = \
            ...
            candidates['item']=item_data
            candidates['user']=user_data
            candidates['popular']=pop_data
    ...
    def evaluate(self, eval_data, load_best_model=True, model_file=None, show_progress=False):
        ...
        candidates = dict()
        data_train, tgt_train, repeat_data, explore_data, item_data, user_data, pop_data = \
            eval_data.get_batch_data(0, eval_data.__len__())
        candidates['repeat'] = repeat_data
        candidates['explore'] = explore_data
        candidates['item'] = item_data
        candidates['user'] = user_data
        candidates['popular'] = pop_data
        scores, _, _, _, _, _ = self.model.forward(data_train, tgt_train, candidates)
...
def main(argv):
    ...
    train_setting = argv[1]
    with open(train_setting, 'r') as f:
        train_config = json.load(f)
    train_config['meta_loss'] = int(argv[2])
    train_config['candidate_num'] = int(argv[3])
    train_config['valid_metric'] = argv[4]
    train_config['data_config_file'] = train_config['data_config_file']+str(argv[5])+'.json'
    train_config['model_name'] = str(argv[2])+'-'+ train_config['model_name']+'-'+argv[4]+'-'+str(argv[3])+'-'+str(argv[5])+'-'
    ...
    validate_dataset = BasketDataset(train_config, mode='meta')
    model = ExplainableNBRNet(train_config, dataset_config)
